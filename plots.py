...
def create_histogram(data, color, label, subplot_position):
    ...
    plt.hist(data, bins=range(1, maxWords + 2, step),
...
train_lengths = [len(review)
                 for review in x_train if limit == None or len(review) <= limit]
test_lengths = [len(review)
                for review in x_test if limit == None or len(review) <= limit]
...
train_hist, bin_edges = np.histogram(
    train_lengths, bins=range(1, maxWords + 2, step))
test_hist, _ = np.histogram(test_lengths, bins=range(1, maxWords + 2, step))
diff_hist = train_hist - test_hist

plt.subplot(1, 3, 3)
plt.bar(bin_edges[:-1], diff_hist, width=1,
        color='purple', alpha=0.7, label='Difference')
